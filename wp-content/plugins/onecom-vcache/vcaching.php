...
Description: With one.com Performance Cache enabled your website loads a lot faster. We save a cached copy of your website on a Varnish server, that will then be served to your next visitors.
Version: 0.2.9
Author: one.com
...
class VCachingOC {
    ...
    public function create_custom_fields()
    {
        if (function_exists('add_meta_box')) {
            foreach ($this->postTypes as $postType) {
                add_meta_box($this->plugin, __('Performance Cache', $this->plugin), array($this, 'display_custom_fields'), $postType, 'side', 'high');
            }
        }
    }
    ...
    {
        return;
        //echo '<div id="message" class="updated fade"><p><strong>' . __('Performance Cache', $this->plugin) . '</strong><br /><br />' . $this->noticeMessage . '</p></div>';
    }
    ...
    public function purge_post_page()
    {
        ...
        /*if (isset($_SESSION['vcaching_note'])) {
            echo '<div id="message" class="updated fade"><p><strong>' . __('Performance Cache', $this->plugin) . '</strong><br /><br />' . $_SESSION['vcaching_note'] . '</p></div>';
    ...
    public function purge_varnish_cache_all_adminbar($admin_bar)
    {
        $admin_bar->add_menu(array(
            ...
            'title' => '<span class="ab-icon dashicons dashicons-trash"></span>' . __('Purge Performance Cache', $this->plugin),
            'href'  => wp_nonce_url(add_query_arg($this->getParam, 1), $this->plugin),
            'meta'  => array(
                'title' => __('Purge Performance Cache', $this->plugin),
            )
        ));
    }
	...
	public function send_headers()
	{
        ...
        if ($enable === "true" || $enable === true || $enable === 1) {
			...
			/*if ($this->debug) {
				...
				error_log('headers sent [ln:544] ==> X-VC-Debug: true');
			}*/
		} else {
    ...
    ?>
        <div class="wrap">
        <h1><?=__('Performance Cache', $this->plugin)?></h1>
    ...
    public function varnish_caching_enable()
    {
        ?>
            ...
            <p class="description"><?=__('Enable Performance Cache', $this->plugin)?></p>
        <?php
    }
