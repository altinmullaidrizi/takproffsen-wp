...
if (! function_exists('mfn_styles')) {
	function mfn_styles()
	{
		...
		if ($google_array) {
			...
			wp_enqueue_style('mfn-fonts', 'https://fonts.googleapis.com/css?family='. $google_array . $subset . '&font-display=swap');
		}
		...
		if( 'custom' == mfn_opts_get( 'button-style' ) ){
			...
			if ( in_array( $button_font_family, $google_fonts ) ) {
				...
				wp_enqueue_style('mfn-font-button', 'https://fonts.googleapis.com/css?family='. $button_font_family . ':400,' . $button_font['weight_style'] . $subset . '&font-display=swap');
			}
